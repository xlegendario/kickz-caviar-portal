 ...
 */
export async function syncSales({ reader, store, windowDays = 60, now = () => new Date(), logger = console }) {
  const startedAt = now();
  ...
  return {
      ...
      for (let i = 0; i < unitIds.length; i += chunk) {
        out.push(
          ...(unwrap(
            await supabase
              .from("api_sales")
              .select("id, unit_record_id, role, fingerprint")
    ...
    async insertMany(rows) {
      for (let i = 0; i < rows.length; i += chunk) {
        unwrap(await supabase.from("api_sales").insert(rows.slice(i, i + chunk)));
      }
    },
