...
/*
 * What a change means to the buyer.
 *
 * Placing one is not an event: whoever placed it already knows, and a
 * want-to-buy placed in the dashboard is picked up by the feed like
 * everything else. The moments a buyer acts on get their own event, sent
 * instead of want_to_buy.updated: a seller found (the invoice amount is
 * known), paid, shipped, delivered.
 */
...
export function deriveWantToBuyEventTypes(previous, next) {
  ...
  if (isOpenWantToBuy(previous) && !isOpenWantToBuy(next) && !isCancelledWantToBuy(next)) types.push("want_to_buy.filled");
  if (previous.payment_status !== "Paid" && next.payment_status === "Paid") types.push("want_to_buy.paid");
  if (!isShippedOrLater(previous.shipping_status) && next.shipping_status === "Shipped") types.push("want_to_buy.shipped");
  if (previous.shipping_status !== "Delivered" && next.shipping_status === "Delivered") types.push("want_to_buy.delivered");

  return types.length ? types : ["want_to_buy.updated"];
}
...
export async function syncWantToBuys({ reader, store, windowDays = 60, emitEvents = false, now = () => new Date(), logger = console }) {
  ...
  const known = new Map(existing.map((row) => [row.wtb_record_id, { ...row }]));
  ...
  return {
    ...
    events: updates.flatMap((row) => {
      const previous = known.get(row.wtb_record_id);
      const item = serializeWantToBuy(row);
      const changes = changedFields(serializeWantToBuy(previous), item);

      return deriveWantToBuyEventTypes(previous, row).map((type) => ({
        type,
