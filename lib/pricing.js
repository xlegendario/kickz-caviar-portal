...
// The net sale price of a Member WTB, from the buyer's Max Price.
//
// REVERTED — a reverse-charge branch was added here and then taken out
// again, because it was wrong. DO NOT ADD IT BACK. The reasoning:
//
// On "All Inventory" the buyer types an ALL-IN price: 21% Dutch VAT is
// inside the number, and the buyer cannot see which VAT type the
// underlying unit has. Stripping that 21% is therefore right for
// EVERYONE, including a reverse-charge buyer — precisely because they
// pay no Dutch VAT, so the VAT sitting inside their typed number has to
// come out.
//
// Reverse-charge requires a VAT ID, so such a buyer is always a
// business. Compare like with like: a Dutch business also reclaims the
// 21%, so its real cost on a typed €200 is €165.29. Leaving the €200
// whole for the foreign business made it pay €200 in real terms — 21%
// more than the Dutch business for the same item at the same typed
// amount. Equal cash out, unequal price.
//
//   VAT0/VAT21 · All Inventory · ANY buyer -> / (1 + buyer's VAT rate)
//   VAT0/VAT21 · B2B Only      · ANY buyer -> bare (Max Price is already
//                                             excl. VAT there)
//   Margin     · any           · ANY buyer -> bare
//
// REFINED — the divisor is the BUYER's own VAT rate, not a fixed 1.21.
// The Buying page states that the member fills in an all-in price, so the
// VAT inside that number is the VAT of THEIR country: a Danish business
// typing 200 means 200 incl. 25%, which is €160 net, not €165.29. This
// mirrors the store side, where an all-in Max Buying Price is converted
// with (1 + Client VAT Rate). For a Dutch buyer the rate is 0.21, so this
// gives the same 1.21 as before — NL is simply the case where the two
// coincide.
//
// The 4th argument is deliberately a plain RATE, not a buyer-context
// object. This function must never branch on "is this reverse-charge":
// that was tried once and was wrong (see the note above about equal cash
// vs equal price). Handing it only a number makes that impossible.
//
// Falls back to 0.21 when the rate is missing, zero or unreadable, which
// reproduces the previous behaviour rather than silently dividing by 1.
export function getMemberWtbNetSalePrice(price, vatType, inventoryFilter, buyerVatRate) {
  ...
  if (type === "VAT0" || type === "VAT21") {
    ...
    const rate = numberValue(buyerVatRate);
    const divisor = rate > 0 ? 1 + rate : 1.21;
...
// What the buyer is actually invoiced, from the net Final Buying Price.
//
// Mirrors the Airtable formula Member WTBs."Invoice Price": add 21% only
// when the unit is VAT0/VAT21 AND the buyer really pays Dutch VAT (NL, or
// no VAT ID at all). A reverse-charge buyer and a Margin unit are billed
// the bare amount.
//
// Duplicating an Airtable formula in code is the exact pattern this audit
// keeps finding bugs in, so it lives here — one place, covered by tests —
// rather than being re-derived at each embed or dashboard. If the Airtable
// formula ever changes, change this with it.
export function memberWtbBuyerInvoiceAmount(finalBuyingPrice, unitVatType, memberWtbFields = {}) {
  const net = Number(finalBuyingPrice);

  if (!Number.isFinite(net) || net <= 0) return 0;

  const type = asText(unitVatType);
  const isVatUnit = type === "VAT0" || type === "VAT21";

  if (isVatUnit && !memberWtbIsReverseCharge(memberWtbFields)) {
    return Math.round(net * 1.21 * 100) / 100;
  }
...
export function calculateCounterPayoutForVatType(storeCounterAllInPrice, vatType, orderFields = {}) {
  ...
  // One rule, one place. See storeMarginInverse above.
  // Not a const: the VAT21 bridge below still adjusts it.
  let payout = storeMarginInverse({
    storePrice: converted,
    method,
    percentage,
    margin,
    cap: numberValue(orderFields["Margin Cap"])
  });
...
/*
 * What we add on top of what a seller is paid, and how to undo it.
 *
 * The rule is one rule, but it was written out in six places: the Airtable
 * "Offer To Store" formula, twice forward in JS, and three times inverted.
 * They agreed only because each was fixed by hand whenever one drifted -
 * the comments around them are a record of exactly that happening.
 *
 * So the arithmetic lives here and nowhere else. Callers keep their own
 * field reading, because a store order carries these as lookups and a
 * flattened record does not, but neither decides what the numbers mean.
 *
 * THE RULE
 *
 *   Firm Range   base + margin
 *   Percentage   max(base + 10, base * (1 + pct) + 5)
 *
 * and, when a cap is set, never more than base + cap on top.
 *
 * The percentage branch therefore has three regimes as the base grows: the
 * flat 10 wins while the base is small, the percentage wins in the middle,
 * and the cap wins once the percentage would ask for more than we allow
 * ourselves. The inverse has to pick the same regime back out of a single
 * number, which is the whole reason this pair sits together.
 */
export function storeMarginForward({ base, method, percentage, margin, cap }) {
  const n = Number(base);

  if (!Number.isFinite(n) || n <= 0) return null;

  const pct = Number(percentage);
  const flat = Number(margin);
  const ceiling = Number(cap);

  let price;

  if (asText(method) === "Firm Range") {
    if (!Number.isFinite(flat)) return null;
    price = n + flat;
  } else if (Number.isFinite(pct) && pct > 0) {
    price = Math.max(n + 10, n * (1 + pct) + 5);
  ...
  if (Number.isFinite(ceiling) && ceiling > 0) {
    price = Math.min(price, n + ceiling);
  }
...
/*
 * The same rule read backwards: given what the store pays, what does that
 * leave for the seller.
 *
 * Checked from the top down, because only the highest regime that still
 * fits can be the one the forward direction used. Getting the order wrong
 * pays a seller the wrong amount and nothing on screen would say so, which
 * is why the tests assert the two are exact inverses rather than close.
 */
export function storeMarginInverse({ storePrice, method, percentage, margin, cap }) {
  ...
  const flat = Number(margin);
  const ceiling = Number(cap);
  const capped = Number.isFinite(ceiling) && ceiling > 0;

  if (asText(method) === "Firm Range") {
    if (!Number.isFinite(flat)) return null;

    // A margin above the cap was never charged in the first place.
    return store - (capped ? Math.min(flat, ceiling) : flat);
  }
  ...
  // The cap binds from the base where the percentage would ask for more
  // than the cap allows: base * pct + 5 >= cap.
  if (capped) {
    const fromCap = store - ceiling;

    if (fromCap >= (ceiling - 5) / pct) return fromCap;
  }

  // The flat 10 wins while base <= 5 / pct, which is where base + 10 and
  // base * (1 + pct) + 5 cross.
  const fromFloor = store - 10;

  if (fromFloor <= 5 / pct) return fromFloor;

  return (store - 5) / (1 + pct);
}
...
export function calculateStoreCounterEquivalent(sellerAskPrice, vatType, orderFields = {}, preAdjustmentMultiplier = 1) {
  ...
  // One rule, one place. See storeMarginForward above.
  const storePrice = storeMarginForward({
    base: converted,
    method,
    percentage,
    margin,
...
const DEFAULT_CONSIGNMENT_MARKUP = (() => {
...
export function getMemberWtbMargin(memberWtbFields = {}) {
  ...
  return configured !== 0 ? configured : DEFAULT_CONSIGNMENT_MARKUP;
}
...
export function calculateMemberWtbBuyerEquivalent(sellerAskPrice, vatType, memberWtbFields = {}) {
  ...
  if (type === "Margin") {
    // CHANGED - the margin is what we keep NET, so on a VAT-inclusive price
    // it is worth margin x 1.21.
    //
    // Every other branch adds it to a net figure and grosses the result up,
    // which is why they all keep the full margin. This one added it straight
    // onto a gross price, so margin stock earned margin / 1.21 - on a 10 that
    // is 8.26, and the shop meanwhile quoted the pair 3 euro higher because
    // getBuyingDisplayPrice already did it correctly.
    //
    // Now the setting means the same everywhere: change it to 7.50 and 7.50
    // net is what it earns, whichever VAT type the pair carries.
    return Math.round((price + margin * 1.21) * 100) / 100;
  }
...
export function calculateMemberWtbSellerPayout(buyerPrice, vatType, memberWtbFields = {}) {
  ...
  if (type === "Margin") {
    // Exact inverse of the branch above, or a counter round would quietly
    // change the deal.
    return Math.round((price - margin * 1.21) * 100) / 100;
  }
