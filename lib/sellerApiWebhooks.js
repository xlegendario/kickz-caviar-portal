...
//
// A specific event is sent instead of the general .updated one, never next
// to it, and every event lists the fields that changed. So a receiver can
// subscribe to exactly the moments it cares about, or to all of them.
...
export const WANT_TO_BUY_EVENTS = [
  "want_to_buy.filled",
  "want_to_buy.paid",
  "want_to_buy.shipped",
  "want_to_buy.delivered",
  "want_to_buy.cancelled",
  "want_to_buy.updated"
];

export const WEBHOOK_EVENTS = [...SALE_EVENTS, ...WANT_TO_BUY_EVENTS];

/*
 * The fields that differ between two serialized items, in item order.
 * updated_at always moves, so it is not a change of its own.
 */
export function changedFields(previousItem, nextItem) {
  if (!previousItem) return [];

  return Object.keys(nextItem).filter(
    (key) => key !== "updated_at" && JSON.stringify(previousItem[key] ?? null) !== JSON.stringify(nextItem[key] ?? null)
  );
}
...
export function buildEventPayload({ eventId, type, item, changes = [], createdAt, test = false }) {
  return {
    id: eventId,
    type,
 ...
 *
 * events: [{ type, partyRecordId, item, changes }]
 */
export async function enqueueEvents({ events, store, now = () => new Date() }) {
  ...
  for (const event of events) {
    ...
    const payload = buildEventPayload({ eventId, type: event.type, item: event.item, changes: event.changes, createdAt });
