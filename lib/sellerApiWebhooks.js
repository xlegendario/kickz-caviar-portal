...
//
// Sale events pushed to a seller's own endpoint.
//
// The sales feed is the guarantee and this is the courtesy: every event sent
// here is also in GET /api/v1/sales, so a delivery that never arrives costs a
...
export const WEBHOOK_EVENTS = ["sale.created", "sale.updated", "sale.cancelled", "label.ready"];
...
export function signatureHeaders({ secret, body, eventType, deliveryId, now = Date.now() }) {
  ...
  return {
    ...
    "User-Agent": "KickzCaviar-Webhooks/1",
    "X-KC-Event": eventType,
    "X-KC-Delivery": deliveryId,
    "X-KC-Timestamp": String(timestamp),
    "X-KC-Signature": `sha256=${signPayload({ secret, timestamp, body })}`
  };
}
