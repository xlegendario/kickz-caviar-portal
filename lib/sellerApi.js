...
async function sendTestEvent({ hook, sellerRecordId, type, send }) {
  ...
  const item = eventType.startsWith("want_to_buy.")
    ? serializeWantToBuy(eventType === "want_to_buy.cancelled" ? { ...sampleWantToBuy(), purchase_status: "Cancelled", fulfillment_status: "Cancelled" } : sampleWantToBuy())
    : serializeSale(sampleSales(sellerRecordId)[0]);
