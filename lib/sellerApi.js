...
export function createSellerApiRouter(deps) {
  ...
  router.patch("/inventory/:id", wrap(async (req, res) => {
    ...
    // Without consignor approval a listing can only be emptied, never
    // restocked: a zeroed row going back to 1 is a new listing by the back
    // door.
    if (patch.quantity > 0) {
      const notConsignor = await consignorRefusal(req.api.sellerRecordId);
