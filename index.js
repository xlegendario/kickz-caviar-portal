...
// NEW — additive only: notifies the store when their very first
// consignment counter (never itself a response to anything) gets
// denied by the consignor, with nothing to reopen. Posts to a new
// endpoint in airtable-discord-updates-main (the store's own Discord
// connection lives there, same as the rest of the consignment
// store-facing flow) showing a plain denial notice with a RETRY
// button, mirroring the existing "Place New Offer" pattern used for
// Store Orders' round-1 denials.
async function notifyStoreOfConsignmentDeadEndDenial(deniedOffer) {
  const orderRecord = await airtable(ORDERS_TABLE).find(deniedOffer.order_record_id);
  const orderFields = orderRecord.fields || {};

  const response = await fetch(`${AIRTABLE_DISCORD_UPDATES_URL}/post-consignment-store-retry-notice`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({
      ...
      offer_id: deniedOffer.id,
      order_id: deniedOffer.order_id,
      product_name: deniedOffer.product_name,
      sku: deniedOffer.sku,
      size: deniedOffer.size,
      denied_price: Number(deniedOffer.store_counter_price)
    })
  });

  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data.error || "Failed to notify store of dead-end consignment denial");
  }
}

// NEW — additive only: the consignor-side counterpart of
// notifyStoreOfConsignmentDeadEndDenial above. Sent directly via
// kickz-caviar-portal-main's own Discord connection (same as every
// other consignor-facing consignment message), respecting the same
// private-channel-vs-DM routing.
async function notifyConsignorOfConsignmentDeadEndDenial(deniedOffer) {
  await initDiscord();

  const sellerRecord = await airtable(SELLERS_TABLE).find(deniedOffer.seller_record_id);
  const seller = normalizeSeller(sellerRecord);
  ...
  if (privateChannelId) {
    ...
    const discordUserId = asText(seller?.discord_id);
    if (!discordUserId) {
      throw new Error(`Missing Discord ID for seller ${seller?.seller_id || deniedOffer.seller_id}`);
    }
    ...
    embeds: [
      {
        title: "❌ Offer Denied",
        description: [
          `**${deniedOffer.product_name || "—"}**`,
          `SKU: ${deniedOffer.sku || "—"}`,
          `Size: ${deniedOffer.size || "—"}`,
          "",
          `Order: ${deniedOffer.order_id || "—"}`,
          "",
          `**Your denied counter**`,
          `€${deniedPrice.toFixed(2)}`,
          "",
          `The store denied your counter offer. Click RETRY to submit a new one.`
        ].join("\n"),
        color: 0xe74c3c
      }
    ],
        ...
        components: [
          {
            ...
            style: 1,
            label: "RETRY",
            custom_id: `consignment_consignor_retry:${deniedOffer.id}`
          }
        ]
      }
    ]
...
function bindConsignmentDiscordButtons(client) {
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (
      ...
      !customId.startsWith("consignment_consignor_retry:") &&
      !customId.startsWith("consignment_consignor_retry_modal:") &&
    ...
    if (customId.startsWith("consignment_counter_deny:")) {
      ...
      try {
        if (deniedOffer?.previous_offer_id) {
          ...
          // NEW — additive only: genuine dead end (this was the
          // store's very first counter, never itself a response to
          // anything). Nothing to reopen, so instead notify the store
          // with a RETRY option — a fresh round, exactly like their
          // original counter, so they can try a different price. Same
          // idea as the existing "Place New Offer" pattern for Store
          // Orders' round-1 denials, adapted to consignment.
          console.log("Consignment reopen: no previous_offer_id on denied offer, notifying store with RETRY:", offerId);

          if (deniedOffer) {
            await notifyStoreOfConsignmentDeadEndDenial(deniedOffer).catch((err) =>
              console.error("Failed to notify store of dead-end consignment denial (non-blocking):", err)
            );
          }
        }
      } catch (reopenErr) {
    ...
    // NEW — additive only: consignor's RETRY button/modal after a
    // dead-end consignment denial (their counter got denied, nothing
    // to reopen). Submits a genuinely fresh counter, notifying the
    // store as if it were a brand new proposal.
    if (customId.startsWith("consignment_consignor_retry:")) {
      const offerId = customId.split(":")[1];

      const modal = {
        title: "New Counter Offer",
        custom_id: `consignment_consignor_retry_modal:${offerId}`,
        components: [
          {
            type: 1,
            components: [
              {
                type: 4,
                custom_id: "retry_price",
                label: "Your new counter (whole number)",
                style: 1,
                min_length: 1,
                max_length: 10,
                required: true
      ...
      await interaction.showModal(modal).catch((err) => {
        console.error("Failed to show consignment_consignor_retry modal:", err);
      });

      return;
    }

    if (customId.startsWith("consignment_consignor_retry_modal:")) {
      const offerId = customId.replace("consignment_consignor_retry_modal:", "");
      const rawAmount = interaction.fields.getTextInputValue("retry_price");
      const retryPrice = Number(String(rawAmount).replace(/[^\d.,-]/g, "").replace(",", "."));
      ...
      const response = await fetch(`${APP_PUBLIC_BASE_URL}/api/consignment/offers/${offerId}/consignor-retry`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ price: retryPrice })
      });
      ...
      if (!response.ok) {
        await interaction.reply({
          content: `❌ ${data.error || "Failed to submit your new counter."}`,
          ephemeral: true
        }).catch(() => {});
        return;
      }
      ...
      await interaction.reply({
        content: `🔁 Your new counter of €${retryPrice.toFixed(2)} was sent to the store.`,
        ephemeral: true
      }).catch(() => {});

      await safeEditInteractionMessage(interaction, {
        content: `🔁 You submitted a new counter of €${retryPrice.toFixed(2)}. Waiting on the store.`,
        embeds: interaction.message.embeds,
        components: []
      }).catch(() => {});
...
// ---------------------------------------------------------------------
// NEW — additive only: store submits a fresh counter after their
// round-1 counter was denied with nothing to reopen (a genuine dead
// end). Requires the new counter to be at least MIN_COUNTER_STEP lower
// than the denied one — same "must actually move" rule used
// everywhere else in these flows — rather than letting someone just
// resubmit the exact same number.
// ---------------------------------------------------------------------
app.post("/api/consignment/offers/:id/store-retry", async (req, res) => {
  try {
    const secret = asText(req.headers["x-kc-secret"]);
    ...
    const deniedPrice = Number(deniedOffer.store_counter_price);

    if (Number.isFinite(deniedPrice) && proposedPrice > deniedPrice - MIN_COUNTER_STEP) {
      return res.status(400).json({
        error: `Your new counter must be at least €${MIN_COUNTER_STEP.toFixed(2)} lower than the denied €${deniedPrice.toFixed(2)}.`
      });
    }
    ...
    const orderFields = orderRecord.fields || {};
    const clientCountry = asText(orderFields["Client Country"]);
    const storeOfferVatType = getStoreOfferVatTypeFromConsignmentVat(deniedOffer.vat_type, clientCountry);
    const proposedExclVat = storeOfferVatType === "VAT21"
      ? Math.round((proposedPrice / 1.21) * 100) / 100
      : proposedPrice;
    ...
    // original round-1 counter (the denied one this replaces was also
    // a round-1 counter with nothing before it).
    const { data: newOffer, error: createError } = await supabase
      .from("consignment_offers")
      .insert({
    ...
    try {
      discordResult = await sendConsignmentCounterOfferDiscordMessage({
        ...
        storeOfferPrice: proposedPrice,
        storeOfferVatType,
        yourPreviousCounter: Number(deniedOffer.seller_price),
        deniedAmount: deniedPrice
      });
      ...
      console.error("Failed to notify consignor of store retry (non-blocking):", err);
    }

    res.json({ ok: true, new_offer_id: newOffer.id, dm_sent: !!discordResult });
  } catch (err) {
    console.error("Failed to process consignment store-retry:", err);
    res.status(500).json({ error: "Failed to process store retry", details: err.message });
  }
});

// ---------------------------------------------------------------------
// NEW — additive only: consignor submits a fresh counter after their
// counter was denied by the store with nothing to reopen (a genuine
// dead end — commonly the very first consignor counter, since that one
// updates the round-1 row in place rather than creating a new row, so
// it never gets its own previous_offer_id). Requires the new counter
// to be at least MIN_COUNTER_STEP lower than the denied one, mirroring
// the original /counter endpoint's price computation exactly.
// ---------------------------------------------------------------------
  ...
  try {
    ...
    try {
      if (offer.previous_offer_id) {
        ...
        // NEW — additive only: genuine dead end, and a common one — the
        // very first consignor counter updates the SAME round-1 row
        // (rather than creating a new one), so that row's own
        // previous_offer_id is empty even though it has a real
        // consignor_counter_price on it. Notify the consignor with a
        // RETRY option instead of leaving them with a dead denial.
        console.log("Consignment reopen: no previous_offer_id on denied offer, notifying consignor with RETRY:", offer.id);

        await notifyConsignorOfConsignmentDeadEndDenial(offer).catch((err) =>
          console.error("Failed to notify consignor of dead-end consignment denial (non-blocking):", err)
        );
      }
    } catch (reopenErr) {
